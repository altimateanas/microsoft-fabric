...
function buildAuthConfig() {
    return {
        method: process.env.AUTH_METHOD ?? "default",
...
async function startHttp() {
    ...
    const { getOAuthConfig, createOAuthMetadataHandler, createTokenValidationMiddleware, } = await import("./auth/oauth-handler.js");
    const config = getOAuthConfig();
    const port = parseInt(process.env.PORT ?? "3000", 10);
    const app = express();
    app.use(express.json());
    // OAuth protected resource metadata
    app.get("/.well-known/oauth-protected-resource", createOAuthMetadataHandler(config));
