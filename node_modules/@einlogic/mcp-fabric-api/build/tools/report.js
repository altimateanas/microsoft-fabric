...
export function registerReportTools(server, fabricClient, powerBIClient, workspaceGuard) {
    server.tool("report_list", "List all reports in a workspace", { workspaceId: z.string().describe("The workspace ID") }, READ, async ({ workspaceId }) => {
        try {
    ...
    server.tool("report_get", "Get details of a specific report", {
        workspaceId: z.string().describe("The workspace ID"),
        reportId: z.string().describe("The report ID"),
    }, READ, async ({ workspaceId, reportId }) => {
        try {
    ...
    server.tool("report_create_definition", "Create a new report in a workspace (long-running operation). Reads PBIR definition files from the specified directory.", {
        workspaceId: z.string().describe("The workspace ID"),
    ...
    server.tool("report_update", "Update a report's name or description", {
        workspaceId: z.string().describe("The workspace ID"),
        reportId: z.string().describe("The report ID"),
        displayName: z.string().optional().describe("New display name"),
        description: z.string().optional().describe("New description"),
    }, WRITE, async ({ workspaceId, reportId, displayName, description }) => {
        try {
            await workspaceGuard.assertWorkspaceAllowed(fabricClient, workspaceId);
            const body = {};
    ...
    server.tool("report_delete", "Delete a report", {
        workspaceId: z.string().describe("The workspace ID"),
        reportId: z.string().describe("The report ID"),
    }, DESTRUCTIVE, async ({ workspaceId, reportId }) => {
        try {
            await workspaceGuard.assertWorkspaceAllowed(fabricClient, workspaceId);
    ...
    server.tool("report_clone", "Clone a report via the Power BI API", {
    ...
    }, WRITE, async ({ workspaceId, reportId, name, targetWorkspaceId, targetModelId }) => {
        try {
            ...
            if (targetModelId)
                body.targetModelId = targetModelId;
            const response = await powerBIClient.post(`/groups/${workspaceId}/reports/${reportId}/Clone`, body);
            return { content: [{ type: "text", text: JSON.stringify(response.data, null, 2) }] };
        }
        catch (error) {
            return formatToolError(error);
    ...
    server.tool("report_export", "Export a report to a file format (PDF, PPTX, PNG, etc.) via the Power BI API. Returns an export ID to check status.", {
        workspaceId: z.string().describe("The workspace ID"),
    ...
    }, WRITE, async ({ workspaceId, reportId, format }) => {
        try {
            const response = await powerBIClient.post(`/groups/${workspaceId}/reports/${reportId}/ExportTo`, { format });
            return { content: [{ type: "text", text: JSON.stringify(response.data, null, 2) }] };
        }
        catch (error) {
            return formatToolError(error);
    ...
    server.tool("report_get_export_status", "Get the status of a report export operation via the Power BI API", {
        workspaceId: z.string().describe("The workspace ID"),
        reportId: z.string().describe("The report ID"),
        exportId: z.string().describe("The export ID from report_export"),
    }, READ, async ({ workspaceId, reportId, exportId }) => {
        try {
            const response = await powerBIClient.get(`/groups/${workspaceId}/reports/${reportId}/exports/${exportId}`);
            return { content: [{ type: "text", text: JSON.stringify(response.data, null, 2) }] };
        }
        catch (error) {
            return formatToolError(error);
    ...
    server.tool("report_get_definition", "Get the full definition of a report (PBIR or PBIR-Legacy format). Writes all definition files (report.json, pages, visuals, etc.) to the specified output directory and returns the list of files written.", {
        workspaceId: z.string().describe("The workspace ID"),
        reportId: z.string().describe("The report ID"),
        outputDirectoryPath: z.string().describe("Directory path where report definition files will be written"),
    }, READ, async ({ workspaceId, reportId, outputDirectoryPath }) => {
        try {
            const response = await fabricClient.post(`/workspaces/${workspaceId}/reports/${reportId}/getDefinition`);
            let parts;
            if (response.lro) {
    ...
    server.tool("report_update_definition", "Update a report's full definition (PBIR or PBIR-Legacy). Reads definition files from the specified directory.", {
        workspaceId: z.string().describe("The workspace ID"),
        reportId: z.string().describe("The report ID"),
        definitionDirectoryPath: z.string().describe("Path to a directory containing report definition files (PBIR format)"),
    }, WRITE, async ({ workspaceId, reportId, definitionDirectoryPath }) => {
        try {
            await workspaceGuard.assertWorkspaceAllowed(fabricClient, workspaceId);
            const resolved = await readFilesFromDirectory(definitionDirectoryPath);
            const body = {
            ...
            const response = await fabricClient.post(`/workspaces/${workspaceId}/reports/${reportId}/updateDefinition`, body);
            if (response.lro) {
                const state = await pollOperation(fabricClient, response.lro.operationId);
    ...
    server.tool("report_rebind", "Rebind a report to a different semantic model/dataset via the Power BI API", {
        workspaceId: z.string().describe("The workspace ID"),
        reportId: z.string().describe("The report ID"),
        datasetId: z.string().describe("The target semantic model/dataset ID to rebind to"),
    }, WRITE, async ({ workspaceId, reportId, datasetId }) => {
        try {
            await workspaceGuard.assertWorkspaceAllowed(fabricClient, workspaceId);
            await powerBIClient.post(`/groups/${workspaceId}/reports/${reportId}/Rebind`, { datasetId });
    ...
    server.tool("report_get_pages", "Get the list of pages in a report via the Power BI API", {
        workspaceId: z.string().describe("The workspace ID"),
        reportId: z.string().describe("The report ID"),
    }, READ, async ({ workspaceId, reportId }) => {
        try {
            const response = await powerBIClient.get(`/groups/${workspaceId}/reports/${reportId}/pages`);
            return { content: [{ type: "text", text: JSON.stringify(response.data, null, 2) }] };
        }
        catch (error) {
            return formatToolError(error);
    ...
    server.tool("report_get_datasources", "Get the data sources used by a report via the Power BI API", {
        workspaceId: z.string().describe("The workspace ID"),
        reportId: z.string().describe("The report ID"),
    }, READ, async ({ workspaceId, reportId }) => {
        try {
            const response = await powerBIClient.get(`/groups/${workspaceId}/reports/${reportId}/datasources`);
            return { content: [{ type: "text", text: JSON.stringify(response.data, null, 2) }] };
        }
        catch (error) {
            return formatToolError(error);
