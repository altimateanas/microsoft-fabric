...
export function registerEventstreamTools(server, fabricClient, workspaceGuard) {
    server.tool("eventstream_list", "List all eventstreams in a workspace", { workspaceId: z.string().describe("The workspace ID") }, READ, async ({ workspaceId }) => {
        try {
    ...
    server.tool("eventstream_get", "Get details of a specific eventstream", {
        workspaceId: z.string().describe("The workspace ID"),
        eventstreamId: z.string().describe("The eventstream ID"),
    }, READ, async ({ workspaceId, eventstreamId }) => {
        try {
    ...
    server.tool("eventstream_create", "Create a new eventstream in a workspace (long-running operation)", {
        workspaceId: z.string().describe("The workspace ID"),
    ...
    server.tool("eventstream_update", "Update an eventstream's name or description", {
        workspaceId: z.string().describe("The workspace ID"),
        eventstreamId: z.string().describe("The eventstream ID"),
        displayName: z.string().optional().describe("New display name"),
        description: z.string().optional().describe("New description"),
    }, WRITE, async ({ workspaceId, eventstreamId, displayName, description }) => {
        try {
            await workspaceGuard.assertWorkspaceAllowed(fabricClient, workspaceId);
            const body = {};
    ...
    server.tool("eventstream_delete", "Delete an eventstream", {
        workspaceId: z.string().describe("The workspace ID"),
        eventstreamId: z.string().describe("The eventstream ID"),
    }, DESTRUCTIVE, async ({ workspaceId, eventstreamId }) => {
        try {
            await workspaceGuard.assertWorkspaceAllowed(fabricClient, workspaceId);
    ...
    server.tool("eventstream_get_definition", "Get the definition of an eventstream (long-running). Writes definition files to the specified output directory and returns the list of files written.", {
        workspaceId: z.string().describe("The workspace ID"),
        eventstreamId: z.string().describe("The eventstream ID"),
        outputDirectoryPath: z.string().describe("Directory path where eventstream definition files will be written"),
    }, READ, async ({ workspaceId, eventstreamId, outputDirectoryPath }) => {
        try {
            const response = await fabricClient.post(`/workspaces/${workspaceId}/eventstreams/${eventstreamId}/getDefinition`);
            let parts;
            if (response.lro) {
    ...
    server.tool("eventstream_update_definition", "Update the definition of an eventstream (long-running). Reads definition files from the specified directory.", {
        workspaceId: z.string().describe("The workspace ID"),
        eventstreamId: z.string().describe("The eventstream ID"),
        definitionDirectoryPath: z.string().describe("Path to a directory containing eventstream definition files"),
    }, WRITE, async ({ workspaceId, eventstreamId, definitionDirectoryPath }) => {
        try {
            await workspaceGuard.assertWorkspaceAllowed(fabricClient, workspaceId);
            ...
            const response = await fabricClient.post(`/workspaces/${workspaceId}/eventstreams/${eventstreamId}/updateDefinition`, body);
            if (response.lro) {
                const state = await pollOperation(fabricClient, response.lro.operationId);
