...
export function registerMirroredDatabaseTools(server, fabricClient, workspaceGuard) {
    server.tool("mirrored_database_list", "List all mirrored databases in a workspace", { workspaceId: z.string().describe("The workspace ID") }, READ, async ({ workspaceId }) => {
        try {
    ...
    server.tool("mirrored_database_get", "Get details of a specific mirrored database", {
        workspaceId: z.string().describe("The workspace ID"),
        mirroredDatabaseId: z.string().describe("The mirrored database ID"),
    }, READ, async ({ workspaceId, mirroredDatabaseId }) => {
        try {
    ...
    server.tool("mirrored_database_create", "Create a new mirrored database in a workspace (long-running)", {
        workspaceId: z.string().describe("The workspace ID"),
    ...
    server.tool("mirrored_database_update", "Update a mirrored database's name or description", {
        workspaceId: z.string().describe("The workspace ID"),
        mirroredDatabaseId: z.string().describe("The mirrored database ID"),
        displayName: z.string().optional().describe("New display name"),
        description: z.string().optional().describe("New description"),
    }, WRITE, async ({ workspaceId, mirroredDatabaseId, displayName, description }) => {
        try {
            await workspaceGuard.assertWorkspaceAllowed(fabricClient, workspaceId);
            const body = {};
    ...
    server.tool("mirrored_database_delete", "Delete a mirrored database", {
        workspaceId: z.string().describe("The workspace ID"),
        mirroredDatabaseId: z.string().describe("The mirrored database ID"),
    }, DESTRUCTIVE, async ({ workspaceId, mirroredDatabaseId }) => {
        try {
            await workspaceGuard.assertWorkspaceAllowed(fabricClient, workspaceId);
    ...
    server.tool("mirrored_database_get_definition", "Get the definition of a mirrored database (long-running). Writes definition files to the specified output directory.", {
        workspaceId: z.string().describe("The workspace ID"),
        mirroredDatabaseId: z.string().describe("The mirrored database ID"),
        outputDirectoryPath: z.string().describe("Directory path where definition files will be written"),
    }, READ, async ({ workspaceId, mirroredDatabaseId, outputDirectoryPath }) => {
        try {
            const response = await fabricClient.post(`/workspaces/${workspaceId}/mirroredDatabases/${mirroredDatabaseId}/getDefinition`);
            let parts;
            if (response.lro) {
    ...
    server.tool("mirrored_database_update_definition", "Update a mirrored database's definition (long-running). Accepts definition parts inline or a directory path.", {
        workspaceId: z.string().describe("The workspace ID"),
        mirroredDatabaseId: z.string().describe("The mirrored database ID"),
    ...
    }, WRITE, async ({ workspaceId, mirroredDatabaseId, parts, partsDirectoryPath }) => {
        try {
            await workspaceGuard.assertWorkspaceAllowed(fabricClient, workspaceId);
            ...
            const response = await fabricClient.post(`/workspaces/${workspaceId}/mirroredDatabases/${mirroredDatabaseId}/updateDefinition`, body);
            if (response.lro) {
                const state = await pollOperation(fabricClient, response.lro.operationId);
    ...
    server.tool("mirrored_database_start_mirroring", "Start mirroring for a mirrored database", {
        workspaceId: z.string().describe("The workspace ID"),
        mirroredDatabaseId: z.string().describe("The mirrored database ID"),
    }, WRITE, async ({ workspaceId, mirroredDatabaseId }) => {
        try {
            await workspaceGuard.assertWorkspaceAllowed(fabricClient, workspaceId);
    ...
    server.tool("mirrored_database_stop_mirroring", "Stop mirroring for a mirrored database", {
        workspaceId: z.string().describe("The workspace ID"),
        mirroredDatabaseId: z.string().describe("The mirrored database ID"),
    }, WRITE, async ({ workspaceId, mirroredDatabaseId }) => {
        try {
    ...
    server.tool("mirrored_database_get_mirroring_status", "Get the mirroring status of a mirrored database", {
        workspaceId: z.string().describe("The workspace ID"),
        mirroredDatabaseId: z.string().describe("The mirrored database ID"),
    }, READ, async ({ workspaceId, mirroredDatabaseId }) => {
        try {
            const response = await fabricClient.get(`/workspaces/${workspaceId}/mirroredDatabases/${mirroredDatabaseId}/getMirroringStatus`);
            return { content: [{ type: "text", text: JSON.stringify(response.data, null, 2) }] };
        }
        catch (error) {
            return formatToolError(error);
    ...
    server.tool("mirrored_database_get_tables_mirroring_status", "Get the mirroring status of individual tables in a mirrored database", {
        workspaceId: z.string().describe("The workspace ID"),
        mirroredDatabaseId: z.string().describe("The mirrored database ID"),
    }, READ, async ({ workspaceId, mirroredDatabaseId }) => {
        try {
            const response = await fabricClient.get(`/workspaces/${workspaceId}/mirroredDatabases/${mirroredDatabaseId}/getTablesMirroringStatus`);
            return { content: [{ type: "text", text: JSON.stringify(response.data, null, 2) }] };
        }
        catch (error) {
            return formatToolError(error);
...
//# sourceMappingURL=mirrored-database.js.map