...
async function fetchNotebookDefinitionParts(fabricClient, workspaceId, notebookId) {
    const response = await fabricClient.post(`/workspaces/${workspaceId}/notebooks/${notebookId}/getDefinition`);
    if (response.lro) {
...
function rewriteIpynbDependencies(content, mutate) {
...
function rewritePyDependencies(content, mutate) {
    const lines = content.split(/\r?\n/);
    const start = lines.findIndex((l) => /^# META \{/.test(l));
    if (start === -1)
        throw new Error("Notebook .py has no `# META` metadata block");
    let end = start;
    while (end < lines.length && /^# META(\s|$)/.test(lines[end]))
        end++;
    const jsonStr = lines
        .slice(start, end)
...
async function updateNotebookDependencies(fabricClient, workspaceId, notebookId, mutate) {
    const parts = await fetchNotebookDefinitionParts(fabricClient, workspaceId, notebookId);
    const idx = parts.findIndex((p) => p.path.endsWith("notebook-content.py") || p.path.endsWith("notebook-content.ipynb"));
    if (idx === -1)
        throw new Error("No notebook-content part found in definition");
    const part = parts[idx];
    const raw = part.payloadType === "InlineBase64" ? decodeBase64(part.payload) : part.payload;
    ...
    const response = await fabricClient.post(`/workspaces/${workspaceId}/notebooks/${notebookId}/updateDefinition`, { definition: { parts } });
    if (response.lro) {
        await pollOperation(fabricClient, response.lro.operationId);
    }
}
export function registerNotebookTools(server, fabricClient, workspaceGuard) {
    server.tool("notebook_list", "List all notebooks in a workspace", { workspaceId: z.string().describe("The workspace ID") }, READ, async ({ workspaceId }) => {
        try {
    ...
    server.tool("notebook_get", "Get details of a specific notebook", {
        workspaceId: z.string().describe("The workspace ID"),
        notebookId: z.string().describe("The notebook ID"),
    }, READ, async ({ workspaceId, notebookId }) => {
        try {
    ...
    server.tool("notebook_create", "Create a new notebook in a workspace (long-running operation)", {
        workspaceId: z.string().describe("The workspace ID"),
    ...
    }, WRITE, async ({ workspaceId, displayName, description }) => {
        try {
    ...
    server.tool("notebook_update", "Update a notebook's name or description", {
        workspaceId: z.string().describe("The workspace ID"),
        notebookId: z.string().describe("The notebook ID"),
        displayName: z.string().optional().describe("New display name"),
        description: z.string().optional().describe("New description"),
    }, WRITE, async ({ workspaceId, notebookId, displayName, description }) => {
        try {
            await workspaceGuard.assertWorkspaceAllowed(fabricClient, workspaceId);
            const body = {};
    ...
    server.tool("notebook_delete", "Delete a notebook", {
        workspaceId: z.string().describe("The workspace ID"),
        notebookId: z.string().describe("The notebook ID"),
    }, DESTRUCTIVE, async ({ workspaceId, notebookId }) => {
        try {
            await workspaceGuard.assertWorkspaceAllowed(fabricClient, workspaceId);
    ...
    server.tool("notebook_get_definition", "Get the content/definition of a notebook (long-running). Writes definition files to the specified output directory.", {
        workspaceId: z.string().describe("The workspace ID"),
        notebookId: z.string().describe("The notebook ID"),
        outputDirectoryPath: z.string().describe("Directory path where notebook definition files will be written"),
    }, READ, async ({ workspaceId, notebookId, outputDirectoryPath }) => {
        try {
            const response = await fabricClient.post(`/workspaces/${workspaceId}/notebooks/${notebookId}/getDefinition`);
            let parts;
            if (response.lro) {
    ...
    server.tool("notebook_update_definition", "Update the content/definition of a notebook (long-running). Reads definition files from the specified directory.", {
        workspaceId: z.string().describe("The workspace ID"),
        notebookId: z.string().describe("The notebook ID"),
        definitionDirectoryPath: z.string().describe("Path to a directory containing notebook definition files"),
    }, WRITE, async ({ workspaceId, notebookId, definitionDirectoryPath }) => {
        try {
            await workspaceGuard.assertWorkspaceAllowed(fabricClient, workspaceId);
            ...
            const response = await fabricClient.post(`/workspaces/${workspaceId}/notebooks/${notebookId}/updateDefinition`, body);
            if (response.lro) {
                const state = await pollOperation(fabricClient, response.lro.operationId);
    ...
    server.tool("notebook_run", "Run a notebook on demand", {
        workspaceId: z.string().describe("The workspace ID"),
        notebookId: z.string().describe("The notebook ID"),
        parameters: z.record(z.unknown()).optional().describe("Notebook parameters as key-value pairs"),
    }, WRITE, async ({ workspaceId, notebookId, parameters }) => {
        try {
    ...
    server.tool("notebook_get_run_status", "Get the status of a notebook run", {
        workspaceId: z.string().describe("The workspace ID"),
        notebookId: z.string().describe("The notebook ID"),
        jobInstanceId: z.string().describe("The job instance ID from notebook_run"),
    }, READ, async ({ workspaceId, notebookId, jobInstanceId }) => {
        try {
    ...
    server.tool("notebook_cancel_run", "Cancel a running notebook execution", {
        workspaceId: z.string().describe("The workspace ID"),
        notebookId: z.string().describe("The notebook ID"),
        jobInstanceId: z.string().describe("The job instance ID to cancel"),
    }, DESTRUCTIVE, async ({ workspaceId, notebookId, jobInstanceId }) => {
        try {
    ...
    server.tool("notebook_attach_environment", "Attach a Fabric Environment to a notebook so it uses that environment's compute/libraries. Mutates the notebook definition metadata (no local files involved).", {
    ...
    }, WRITE, async ({ workspaceId, notebookId, environmentId, environmentWorkspaceId }) => {
        try {
            await workspaceGuard.assertWorkspaceAllowed(fabricClient, workspaceId);
    ...
    server.tool("notebook_detach_environment", "Remove the attached environment from a notebook by deleting the environment binding from its definition metadata.", {
        workspaceId: z.string().describe("The workspace ID"),
        notebookId: z.string().describe("The notebook ID"),
    }, DESTRUCTIVE, async ({ workspaceId, notebookId }) => {
        try {
