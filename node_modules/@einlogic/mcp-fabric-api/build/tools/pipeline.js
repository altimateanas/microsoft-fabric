...
export function registerPipelineTools(server, fabricClient, workspaceGuard) {
    server.tool("pipeline_list", "List all data pipelines in a workspace", { workspaceId: z.string().describe("The workspace ID") }, READ, async ({ workspaceId }) => {
        try {
    ...
    server.tool("pipeline_get", "Get details of a specific data pipeline", {
        workspaceId: z.string().describe("The workspace ID"),
        pipelineId: z.string().describe("The pipeline ID"),
    }, READ, async ({ workspaceId, pipelineId }) => {
        try {
    ...
    server.tool("pipeline_create", "Create a new data pipeline in a workspace", {
        workspaceId: z.string().describe("The workspace ID"),
    ...
    server.tool("pipeline_update", "Update a pipeline's name or description", {
        workspaceId: z.string().describe("The workspace ID"),
        pipelineId: z.string().describe("The pipeline ID"),
        displayName: z.string().optional().describe("New display name"),
        description: z.string().optional().describe("New description"),
    }, WRITE, async ({ workspaceId, pipelineId, displayName, description }) => {
        try {
            await workspaceGuard.assertWorkspaceAllowed(fabricClient, workspaceId);
            const body = {};
    ...
    server.tool("pipeline_delete", "Delete a data pipeline", {
        workspaceId: z.string().describe("The workspace ID"),
        pipelineId: z.string().describe("The pipeline ID"),
    }, DESTRUCTIVE, async ({ workspaceId, pipelineId }) => {
        try {
            await workspaceGuard.assertWorkspaceAllowed(fabricClient, workspaceId);
    ...
    server.tool("pipeline_run", "Run a data pipeline on demand", {
        workspaceId: z.string().describe("The workspace ID"),
        pipelineId: z.string().describe("The pipeline ID"),
        parameters: z.record(z.unknown()).optional().describe("Pipeline parameters as key-value pairs"),
    }, WRITE, async ({ workspaceId, pipelineId, parameters }) => {
        try {
    ...
    server.tool("pipeline_get_run_status", "Get the status of a pipeline run", {
        workspaceId: z.string().describe("The workspace ID"),
        pipelineId: z.string().describe("The pipeline ID"),
        jobInstanceId: z.string().describe("The job instance ID from pipeline_run"),
    }, READ, async ({ workspaceId, pipelineId, jobInstanceId }) => {
        try {
    ...
    server.tool("pipeline_cancel_run", "Cancel a running pipeline execution", {
        workspaceId: z.string().describe("The workspace ID"),
        pipelineId: z.string().describe("The pipeline ID"),
        jobInstanceId: z.string().describe("The job instance ID to cancel"),
    }, DESTRUCTIVE, async ({ workspaceId, pipelineId, jobInstanceId }) => {
        try {
    ...
    server.tool("pipeline_list_runs", "List all run instances for a pipeline", {
        workspaceId: z.string().describe("The workspace ID"),
        pipelineId: z.string().describe("The pipeline ID"),
    }, READ, async ({ workspaceId, pipelineId }) => {
        try {
    ...
    // Schedule tools
    server.tool("pipeline_list_schedules", "List all schedules for a pipeline", {
        workspaceId: z.string().describe("The workspace ID"),
        pipelineId: z.string().describe("The pipeline ID"),
    }, READ, async ({ workspaceId, pipelineId }) => {
        try {
            const response = await fabricClient.get(`/workspaces/${workspaceId}/items/${pipelineId}/jobSchedules`);
            return { content: [{ type: "text", text: JSON.stringify(response.data, null, 2) }] };
        }
        catch (error) {
            return formatToolError(error);
    ...
    server.tool("pipeline_create_schedule", "Create a schedule for a pipeline", {
        workspaceId: z.string().describe("The workspace ID"),
        pipelineId: z.string().describe("The pipeline ID"),
    ...
    }, WRITE, async ({ workspaceId, pipelineId, startDateTime, endDateTime, localTimeZoneId, type, interval, weekDays, times, enabled }) => {
        try {
            await workspaceGuard.assertWorkspaceAllowed(fabricClient, workspaceId);
            const configuration = { type, startDateTime };
            if (endDateTime)
                configuration.endDateTime = endDateTime;
            if (localTimeZoneId)
                configuration.localTimeZoneId = localTimeZoneId;
            if (interval)
                configuration.interval = interval;
            if (weekDays)
                configuration.weekDays = weekDays;
            if (times)
            ...
            const response = await fabricClient.post(`/workspaces/${workspaceId}/items/${pipelineId}/jobSchedules`, body);
            return { content: [{ type: "text", text: JSON.stringify(response.data, null, 2) }] };
        }
        catch (error) {
            return formatToolError(error);
    ...
    server.tool("pipeline_update_schedule", "Update an existing pipeline schedule", {
        workspaceId: z.string().describe("The workspace ID"),
        pipelineId: z.string().describe("The pipeline ID"),
    ...
    }, WRITE, async ({ workspaceId, pipelineId, scheduleId, enabled, configuration }) => {
        try {
            await workspaceGuard.assertWorkspaceAllowed(fabricClient, workspaceId);
            const body = {};
            ...
            const response = await fabricClient.patch(`/workspaces/${workspaceId}/items/${pipelineId}/jobSchedules/${scheduleId}`, body);
            return { content: [{ type: "text", text: JSON.stringify(response.data, null, 2) }] };
        }
        catch (error) {
            return formatToolError(error);
    ...
    server.tool("pipeline_delete_schedule", "Delete a pipeline schedule", {
        workspaceId: z.string().describe("The workspace ID"),
        pipelineId: z.string().describe("The pipeline ID"),
        scheduleId: z.string().describe("The schedule ID to delete"),
    }, DESTRUCTIVE, async ({ workspaceId, pipelineId, scheduleId }) => {
        try {
            await workspaceGuard.assertWorkspaceAllowed(fabricClient, workspaceId);
            await fabricClient.delete(`/workspaces/${workspaceId}/items/${pipelineId}/jobSchedules/${scheduleId}`);
    ...
    server.tool("pipeline_get_definition", "Get the definition of a data pipeline (long-running). Writes definition files to the specified output directory.", {
        workspaceId: z.string().describe("The workspace ID"),
        pipelineId: z.string().describe("The pipeline ID"),
        outputDirectoryPath: z.string().describe("Directory path where definition files will be written"),
    }, READ, async ({ workspaceId, pipelineId, outputDirectoryPath }) => {
        try {
            const response = await fabricClient.post(`/workspaces/${workspaceId}/dataPipelines/${pipelineId}/getDefinition`);
            let parts;
            if (response.lro) {
    ...
    server.tool("pipeline_update_definition", "Update a data pipeline's definition (long-running). Reads definition files from the specified directory.", {
        workspaceId: z.string().describe("The workspace ID"),
        pipelineId: z.string().describe("The pipeline ID"),
        definitionDirectoryPath: z.string().describe("Path to a directory containing pipeline definition files"),
    }, WRITE, async ({ workspaceId, pipelineId, definitionDirectoryPath }) => {
        try {
            await workspaceGuard.assertWorkspaceAllowed(fabricClient, workspaceId);
            ...
            const response = await fabricClient.post(`/workspaces/${workspaceId}/dataPipelines/${pipelineId}/updateDefinition`, body);
            if (response.lro) {
                const state = await pollOperation(fabricClient, response.lro.operationId);
