...
export function registerGitIntegrationTools(server, fabricClient, workspaceGuard) {
    server.tool("git_get_connection", "Get the Git connection details for a workspace", { workspaceId: z.string().describe("The workspace ID") }, READ, async ({ workspaceId }) => {
        try {
    ...
    server.tool("git_get_status", "Get the Git status of items in a workspace (shows sync state between workspace and remote)", { workspaceId: z.string().describe("The workspace ID") }, READ, async ({ workspaceId }) => {
        try {
    ...
    server.tool("git_connect", "Connect a workspace to a Git repository (Azure DevOps or GitHub)", {
    ...
    }, WRITE, async ({ workspaceId, gitProviderDetails }) => {
        try {
    ...
    server.tool("git_disconnect", "Disconnect a workspace from its Git repository", { workspaceId: z.string().describe("The workspace ID") }, DESTRUCTIVE, async ({ workspaceId }) => {
        try {
            await workspaceGuard.assertWorkspaceAllowed(fabricClient, workspaceId);
    ...
    server.tool("git_initialize_connection", "Initialize a Git connection after connecting (long-running). Chooses whether workspace or remote content takes precedence.", {
        workspaceId: z.string().describe("The workspace ID"),
    ...
    }, WRITE, async ({ workspaceId, initializationStrategy }) => {
        try {
            await workspaceGuard.assertWorkspaceAllowed(fabricClient, workspaceId);
    ...
    server.tool("git_commit_to_git", "Commit workspace changes to the connected Git repository (long-running)", {
        workspaceId: z.string().describe("The workspace ID"),
    ...
    }, WRITE, async ({ workspaceId, mode, comment, items, workspaceHead }) => {
        try {
            await workspaceGuard.assertWorkspaceAllowed(fabricClient, workspaceId);
            const body = { mode };
            if (comment)
            ...
            if (items)
                body.items = items;
            if (workspaceHead)
                body.workspaceHead = workspaceHead;
    ...
    server.tool("git_update_from_git", "Update workspace from the connected Git repository (long-running)", {
        workspaceId: z.string().describe("The workspace ID"),
    ...
    }, WRITE, async ({ workspaceId, remoteCommitHash, conflictResolution, workspaceHead, options }) => {
        try {
            await workspaceGuard.assertWorkspaceAllowed(fabricClient, workspaceId);
            const body = { remoteCommitHash };
    ...
    server.tool("git_get_credentials", "Get the Git credentials configuration for the current user in a workspace", { workspaceId: z.string().describe("The workspace ID") }, READ, async ({ workspaceId }) => {
        try {
    ...
    server.tool("git_update_credentials", "Update the Git credentials configuration for the current user in a workspace", {
        workspaceId: z.string().describe("The workspace ID"),
    ...
    }, WRITE, async ({ workspaceId, source, connectionId }) => {
        try {
            await workspaceGuard.assertWorkspaceAllowed(fabricClient, workspaceId);
            const body = { source };
            if (connectionId)
