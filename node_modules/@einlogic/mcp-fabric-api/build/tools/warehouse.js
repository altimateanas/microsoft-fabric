...
export function registerWarehouseTools(server, fabricClient, workspaceGuard) {
    server.tool("warehouse_list", "List all warehouses in a workspace", { workspaceId: z.string().describe("The workspace ID") }, READ, async ({ workspaceId }) => {
        try {
    ...
    server.tool("warehouse_get", "Get details of a specific warehouse including connection string and provisioning status", {
        workspaceId: z.string().describe("The workspace ID"),
        warehouseId: z.string().describe("The warehouse ID"),
    }, READ, async ({ workspaceId, warehouseId }) => {
        try {
    ...
    server.tool("warehouse_create", "Create a new warehouse in a workspace (long-running operation)", {
        workspaceId: z.string().describe("The workspace ID"),
    ...
    server.tool("warehouse_update", "Update a warehouse's name or description", {
        workspaceId: z.string().describe("The workspace ID"),
        warehouseId: z.string().describe("The warehouse ID"),
        displayName: z.string().optional().describe("New display name"),
        description: z.string().optional().describe("New description"),
    }, WRITE, async ({ workspaceId, warehouseId, displayName, description }) => {
        try {
            await workspaceGuard.assertWorkspaceAllowed(fabricClient, workspaceId);
            const body = {};
    ...
    server.tool("warehouse_delete", "Delete a warehouse", {
        workspaceId: z.string().describe("The workspace ID"),
        warehouseId: z.string().describe("The warehouse ID"),
    }, DESTRUCTIVE, async ({ workspaceId, warehouseId }) => {
        try {
            await workspaceGuard.assertWorkspaceAllowed(fabricClient, workspaceId);
    ...
    server.tool("warehouse_get_sql_endpoint", "Get the SQL connection details for a warehouse", {
        workspaceId: z.string().describe("The workspace ID"),
        warehouseId: z.string().describe("The warehouse ID"),
    }, READ, async ({ workspaceId, warehouseId }) => {
        try {
            ...
            const connectionString = properties?.connectionString;
            const provisioningStatus = properties?.provisioningStatus;
            if (!connectionString) {
    ...
    server.tool("warehouse_list_tables", "List all tables in a warehouse", {
        workspaceId: z.string().describe("The workspace ID"),
        warehouseId: z.string().describe("The warehouse ID"),
    }, READ, async ({ workspaceId, warehouseId }) => {
        try {
    ...
    server.tool("warehouse_get_definition", "Get the definition of a warehouse (long-running). Writes definition files to the specified output directory.", {
        workspaceId: z.string().describe("The workspace ID"),
        warehouseId: z.string().describe("The warehouse ID"),
        outputDirectoryPath: z.string().describe("Directory path where definition files will be written"),
    }, READ, async ({ workspaceId, warehouseId, outputDirectoryPath }) => {
        try {
            const response = await fabricClient.post(`/workspaces/${workspaceId}/warehouses/${warehouseId}/getDefinition`);
            let parts;
            if (response.lro) {
    ...
    server.tool("warehouse_update_definition", "Update a warehouse's definition (long-running). Accepts definition parts inline or a directory path.", {
        workspaceId: z.string().describe("The workspace ID"),
        warehouseId: z.string().describe("The warehouse ID"),
    ...
    }, WRITE, async ({ workspaceId, warehouseId, parts, partsDirectoryPath }) => {
        try {
            await workspaceGuard.assertWorkspaceAllowed(fabricClient, workspaceId);
            ...
            const response = await fabricClient.post(`/workspaces/${workspaceId}/warehouses/${warehouseId}/updateDefinition`, body);
            if (response.lro) {
                const state = await pollOperation(fabricClient, response.lro.operationId);
