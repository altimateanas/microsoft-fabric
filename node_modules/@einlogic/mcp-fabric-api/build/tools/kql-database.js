...
export function registerKqlDatabaseTools(server, fabricClient, workspaceGuard) {
    server.tool("kql_database_list", "List all KQL databases in a workspace", { workspaceId: z.string().describe("The workspace ID") }, READ, async ({ workspaceId }) => {
        try {
    ...
    server.tool("kql_database_get", "Get details of a specific KQL database", {
        workspaceId: z.string().describe("The workspace ID"),
        kqlDatabaseId: z.string().describe("The KQL database ID"),
    }, READ, async ({ workspaceId, kqlDatabaseId }) => {
        try {
    ...
    server.tool("kql_database_create", "Create a new KQL database in a workspace (long-running). Requires a parent eventhouse.", {
        workspaceId: z.string().describe("The workspace ID"),
    ...
    }, WRITE, async ({ workspaceId, displayName, description, databaseType, parentEventhouseItemId }) => {
        try {
    ...
    server.tool("kql_database_update", "Update a KQL database's name or description", {
        workspaceId: z.string().describe("The workspace ID"),
        kqlDatabaseId: z.string().describe("The KQL database ID"),
        displayName: z.string().optional().describe("New display name"),
        description: z.string().optional().describe("New description"),
    }, WRITE, async ({ workspaceId, kqlDatabaseId, displayName, description }) => {
        try {
            await workspaceGuard.assertWorkspaceAllowed(fabricClient, workspaceId);
            const body = {};
    ...
    server.tool("kql_database_delete", "Delete a KQL database", {
        workspaceId: z.string().describe("The workspace ID"),
        kqlDatabaseId: z.string().describe("The KQL database ID"),
    }, DESTRUCTIVE, async ({ workspaceId, kqlDatabaseId }) => {
        try {
            await workspaceGuard.assertWorkspaceAllowed(fabricClient, workspaceId);
    ...
    server.tool("kql_database_get_definition", "Get the definition of a KQL database (long-running). Writes definition files to the specified output directory.", {
        workspaceId: z.string().describe("The workspace ID"),
        kqlDatabaseId: z.string().describe("The KQL database ID"),
        outputDirectoryPath: z.string().describe("Directory path where definition files will be written"),
    }, READ, async ({ workspaceId, kqlDatabaseId, outputDirectoryPath }) => {
        try {
            const response = await fabricClient.post(`/workspaces/${workspaceId}/kqlDatabases/${kqlDatabaseId}/getDefinition`);
            let parts;
            if (response.lro) {
    ...
    server.tool("kql_database_update_definition", "Update a KQL database's definition (long-running). Accepts definition parts inline or a directory path.", {
        workspaceId: z.string().describe("The workspace ID"),
        kqlDatabaseId: z.string().describe("The KQL database ID"),
    ...
    }, WRITE, async ({ workspaceId, kqlDatabaseId, parts, partsDirectoryPath }) => {
        try {
            await workspaceGuard.assertWorkspaceAllowed(fabricClient, workspaceId);
            ...
            const response = await fabricClient.post(`/workspaces/${workspaceId}/kqlDatabases/${kqlDatabaseId}/updateDefinition`, body);
            if (response.lro) {
                const state = await pollOperation(fabricClient, response.lro.operationId);
...
//# sourceMappingURL=kql-database.js.map