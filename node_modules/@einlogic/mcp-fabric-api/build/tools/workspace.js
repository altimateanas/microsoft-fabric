...
export function registerWorkspaceTools(server, fabricClient, workspaceGuard) {
    server.tool("workspace_list", "List all accessible Fabric workspaces", {}, READ, async () => {
        try {
    ...
    server.tool("workspace_get", "Get details of a specific workspace", { workspaceId: z.string().describe("The workspace ID") }, READ, async ({ workspaceId }) => {
        try {
    ...
    server.tool("workspace_create", "Create a new Fabric workspace", {
    ...
    }, WRITE, async ({ displayName, description, capacityId }) => {
        try {
    ...
    server.tool("workspace_update", "Update a workspace's name or description", {
    ...
    server.tool("workspace_delete", "Delete a workspace", { workspaceId: z.string().describe("The workspace ID") }, DESTRUCTIVE, async ({ workspaceId }) => {
        try {
            await workspaceGuard.assertWorkspaceAllowed(fabricClient, workspaceId);
    ...
    server.tool("workspace_list_items", "List all items in a workspace, optionally filtered by type", {
        workspaceId: z.string().describe("The workspace ID"),
        type: z.string().optional().describe("Filter by item type (e.g., Lakehouse, Notebook, Pipeline)"),
    }, READ, async ({ workspaceId, type }) => {
        try {
