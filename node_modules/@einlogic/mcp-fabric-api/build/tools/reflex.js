...
export function registerReflexTools(server, fabricClient, workspaceGuard) {
    server.tool("reflex_list", "List all Reflex (Activator) items in a workspace", { workspaceId: z.string().describe("The workspace ID") }, READ, async ({ workspaceId }) => {
        try {
    ...
    server.tool("reflex_get", "Get details of a specific Reflex (Activator) item", {
        workspaceId: z.string().describe("The workspace ID"),
        reflexId: z.string().describe("The reflex/activator ID"),
    }, READ, async ({ workspaceId, reflexId }) => {
        try {
    ...
    server.tool("reflex_create", "Create a new Reflex (Activator) item in a workspace", {
        workspaceId: z.string().describe("The workspace ID"),
        ...
        try {
            ...
            const body = { displayName, type: "Reflex" };
            if (description)
    ...
    server.tool("reflex_update", "Update a Reflex (Activator) item's name or description", {
        workspaceId: z.string().describe("The workspace ID"),
        reflexId: z.string().describe("The reflex/activator ID"),
        displayName: z.string().optional().describe("New display name"),
        description: z.string().optional().describe("New description"),
    }, WRITE, async ({ workspaceId, reflexId, displayName, description }) => {
        try {
            await workspaceGuard.assertWorkspaceAllowed(fabricClient, workspaceId);
            const body = {};
    ...
    server.tool("reflex_delete", "Delete a Reflex (Activator) item", {
        workspaceId: z.string().describe("The workspace ID"),
        reflexId: z.string().describe("The reflex/activator ID"),
    }, DESTRUCTIVE, async ({ workspaceId, reflexId }) => {
        try {
            await workspaceGuard.assertWorkspaceAllowed(fabricClient, workspaceId);
    ...
    server.tool("reflex_get_definition", "Get the definition of a Reflex (Activator) item (long-running). Writes definition files to the specified output directory and returns the list of files written.", {
        workspaceId: z.string().describe("The workspace ID"),
        reflexId: z.string().describe("The reflex/activator ID"),
        outputDirectoryPath: z.string().describe("Directory path where Reflex definition files will be written"),
    }, READ, async ({ workspaceId, reflexId, outputDirectoryPath }) => {
        try {
            const response = await fabricClient.post(`/workspaces/${workspaceId}/items/${reflexId}/getDefinition`);
            let parts;
            if (response.lro) {
    ...
    server.tool("reflex_update_definition", "Update a Reflex (Activator) item's definition (long-running). Accepts definition parts inline or a directory path.", {
        workspaceId: z.string().describe("The workspace ID"),
        reflexId: z.string().describe("The reflex/activator ID"),
    ...
    }, WRITE, async ({ workspaceId, reflexId, parts, partsDirectoryPath }) => {
        try {
            await workspaceGuard.assertWorkspaceAllowed(fabricClient, workspaceId);
            ...
            const response = await fabricClient.post(`/workspaces/${workspaceId}/items/${reflexId}/updateDefinition`, body);
            if (response.lro) {
