...
export function registerAuthTools(server, tokenManager) {
    server.tool("auth_get_current_account", "Show the current Azure identity, tenant, and token expiry by decoding the Fabric JWT", {}, READ, async () => {
        try {
    ...
    server.tool("auth_list_available_accounts", "List Azure subscriptions and tenants the local user has logged into via 'az login'. Reads local CLI state only — does not query Entra ID or list other users.", {}, READ, async () => {
        try {
            ...
            if (message.includes("ENOENT") || message.includes("not found") || message.includes("not recognized")) {
                return formatToolError(new Error("Azure CLI ('az') is not installed or not on PATH. Install it from https://aka.ms/installazurecli"));
            }
            if (message.includes("az login") || message.includes("Please run")) {
                return formatToolError(new Error("Not logged in to Azure CLI. Run 'az login' first."));
            }
            return formatToolError(error);
        }
    });
    server.tool("auth_switch_tenant", "Switch to a different Azure tenant. Acquires a new token to verify access; rolls back on failure.", {
        tenantId: z.string().describe("The Azure AD tenant ID (GUID) to switch to"),
    }, WRITE, async ({ tenantId }) => {
        ...
        try {
            ...
            const message = error instanceof Error ? error.message : String(error);
            return formatToolError(new Error(`Failed to switch to tenant ${tenantId}. Rolled back to ${previousTenantId ?? "default"}.\nDetails: ${message}`));
        }
    });
    server.tool("auth_clear_token_cache", "Clear cached authentication tokens to force re-acquisition on next API call", {}, DESTRUCTIVE, async () => {
        try {
