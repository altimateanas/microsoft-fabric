...
function isSchemaEnabledError(error) {
...
export function registerLakehouseTools(server, fabricClient, sqlClient, workspaceGuard) {
    server.tool("lakehouse_list", "List all lakehouses in a workspace", { workspaceId: z.string().describe("The workspace ID") }, READ, async ({ workspaceId }) => {
        try {
    ...
    server.tool("lakehouse_get", "Get details of a specific lakehouse including SQL endpoint and OneLake paths", {
        workspaceId: z.string().describe("The workspace ID"),
        lakehouseId: z.string().describe("The lakehouse ID"),
    }, READ, async ({ workspaceId, lakehouseId }) => {
        try {
    ...
    server.tool("lakehouse_create", "Create a new lakehouse in a workspace (long-running operation). Schemas are enabled by default (preview).", {
        workspaceId: z.string().describe("The workspace ID"),
    ...
    }, WRITE, async ({ workspaceId, displayName, description, enableSchemas }) => {
        try {
    ...
    server.tool("lakehouse_update", "Update a lakehouse's name or description", {
        workspaceId: z.string().describe("The workspace ID"),
        lakehouseId: z.string().describe("The lakehouse ID"),
        displayName: z.string().optional().describe("New display name"),
        description: z.string().optional().describe("New description"),
    }, WRITE, async ({ workspaceId, lakehouseId, displayName, description }) => {
        try {
            await workspaceGuard.assertWorkspaceAllowed(fabricClient, workspaceId);
            const body = {};
    ...
    server.tool("lakehouse_delete", "Delete a lakehouse", {
        workspaceId: z.string().describe("The workspace ID"),
        lakehouseId: z.string().describe("The lakehouse ID"),
    }, DESTRUCTIVE, async ({ workspaceId, lakehouseId }) => {
        try {
            await workspaceGuard.assertWorkspaceAllowed(fabricClient, workspaceId);
    ...
    server.tool("lakehouse_list_tables", "List all tables in a lakehouse. For schema-enabled lakehouses, automatically falls back to querying INFORMATION_SCHEMA via the SQL endpoint.", {
        workspaceId: z.string().describe("The workspace ID"),
        lakehouseId: z.string().describe("The lakehouse ID"),
    }, READ, async ({ workspaceId, lakehouseId }) => {
        try {
            ...
            try {
                ...
                const sqlEndpointProps = properties?.sqlEndpointProperties;
                const connectionString = sqlEndpointProps?.connectionString;
                if (!connectionString) {
                    return {
                ...
                const result = await sqlClient.executeQuery(server, dbName, "SELECT TABLE_SCHEMA, TABLE_NAME, TABLE_TYPE FROM INFORMATION_SCHEMA.TABLES ORDER BY TABLE_SCHEMA, TABLE_NAME");
                return {
                    content: [{
                            type: "text",
    ...
    server.tool("lakehouse_load_table", "Load data into a lakehouse table from a file path (long-running operation). Not supported for schema-enabled lakehouses — use sql_endpoint_execute_query with COPY INTO or notebooks instead.", {
        workspaceId: z.string().describe("The workspace ID"),
        lakehouseId: z.string().describe("The lakehouse ID"),
    ...
    }, WRITE, async ({ workspaceId, lakehouseId, tableName, relativePath, pathType, mode, formatOptions }) => {
        try {
            await workspaceGuard.assertWorkspaceAllowed(fabricClient, workspaceId);
            const body = { relativePath, pathType };
            if (mode)
                body.mode = mode;
            if (formatOptions)
                body.formatOptions = formatOptions;
            const response = await fabricClient.post(`/workspaces/${workspaceId}/lakehouses/${lakehouseId}/tables/${tableName}/load`, body);
            if (response.lro) {
                const state = await pollOperation(fabricClient, response.lro.operationId);
    ...
    server.tool("lakehouse_create_shortcut", "Create a OneLake shortcut in a lakehouse at any level (file, folder, table, or schema). " +
        ...
        path: z.string().describe("Full path where the shortcut is created. Must start with 'Files' or 'Tables'. " +
    ...
    }, WRITE, async ({ workspaceId, lakehouseId, name, path, shortcutConflictPolicy, target }) => {
        try {
            await workspaceGuard.assertWorkspaceAllowed(fabricClient, workspaceId);
            const targetKeys = Object.keys(target).filter((k) => target[k] !== undefined);
            if (targetKeys.length !== 1) {
                return {
                    content: [{ type: "text", text: "Error: Specify exactly one target type (oneLake, adlsGen2, amazonS3, googleCloudStorage, s3Compatible, dataverse, azureBlobStorage, or oneDriveSharePoint)" }],
                    isError: true,
                };
            }
    ...
    server.tool("lakehouse_get_sql_endpoint", "Get the SQL endpoint details for a lakehouse", {
        workspaceId: z.string().describe("The workspace ID"),
        lakehouseId: z.string().describe("The lakehouse ID"),
    }, READ, async ({ workspaceId, lakehouseId }) => {
        try {
    ...
    server.tool("lakehouse_get_definition", "Get the definition of a lakehouse (long-running). Writes definition files to the specified output directory.", {
        workspaceId: z.string().describe("The workspace ID"),
        lakehouseId: z.string().describe("The lakehouse ID"),
        outputDirectoryPath: z.string().describe("Directory path where definition files will be written"),
    }, READ, async ({ workspaceId, lakehouseId, outputDirectoryPath }) => {
        try {
            const response = await fabricClient.post(`/workspaces/${workspaceId}/lakehouses/${lakehouseId}/getDefinition`);
            let parts;
            if (response.lro) {
    ...
    server.tool("lakehouse_update_definition", "Update a lakehouse's definition (long-running). Accepts definition parts inline or a directory path.", {
        workspaceId: z.string().describe("The workspace ID"),
        lakehouseId: z.string().describe("The lakehouse ID"),
    ...
    }, WRITE, async ({ workspaceId, lakehouseId, parts, partsDirectoryPath }) => {
        try {
            await workspaceGuard.assertWorkspaceAllowed(fabricClient, workspaceId);
            const resolved = await resolveFilesOrDirectory(parts, partsDirectoryPath);
            const body = {
                definition: {
            ...
            const response = await fabricClient.post(`/workspaces/${workspaceId}/lakehouses/${lakehouseId}/updateDefinition`, body);
            if (response.lro) {
                const state = await pollOperation(fabricClient, response.lro.operationId);
                return { content: [{ type: "text", text: JSON.stringify(state, null, 2) }] };
            }
    ...
    server.tool("lakehouse_list_shortcuts", "List all OneLake shortcuts in a lakehouse", {
        workspaceId: z.string().describe("The workspace ID"),
        lakehouseId: z.string().describe("The lakehouse ID"),
    }, READ, async ({ workspaceId, lakehouseId }) => {
        try {
    ...
    server.tool("lakehouse_get_shortcut", "Get details of a specific OneLake shortcut in a lakehouse", {
        workspaceId: z.string().describe("The workspace ID"),
        lakehouseId: z.string().describe("The lakehouse ID"),
    ...
    }, READ, async ({ workspaceId, lakehouseId, shortcutPath, shortcutName }) => {
        try {
            const response = await fabricClient.get(`/workspaces/${workspaceId}/items/${lakehouseId}/shortcuts/${shortcutPath}/${shortcutName}`);
            return { content: [{ type: "text", text: JSON.stringify(response.data, null, 2) }] };
        }
        catch (error) {
            return formatToolError(error);
    ...
    server.tool("lakehouse_delete_shortcut", "Delete a OneLake shortcut from a lakehouse", {
        workspaceId: z.string().describe("The workspace ID"),
        lakehouseId: z.string().describe("The lakehouse ID"),
        shortcutPath: z.string().describe("The shortcut path (e.g., 'Tables' or 'Files/landingZone')"),
        shortcutName: z.string().describe("The shortcut name"),
    }, DESTRUCTIVE, async ({ workspaceId, lakehouseId, shortcutPath, shortcutName }) => {
        try {
            await workspaceGuard.assertWorkspaceAllowed(fabricClient, workspaceId);
            await fabricClient.delete(`/workspaces/${workspaceId}/items/${lakehouseId}/shortcuts/${shortcutPath}/${shortcutName}`);
...
//# sourceMappingURL=lakehouse.js.map