...
import { runOnDemandJob, getJobInstance } from "../core/job-scheduler.js";
...
export function registerDataflowTools(server, fabricClient, workspaceGuard) {
    server.tool("dataflow_list", "List all Dataflow Gen2 items in a workspace", { workspaceId: z.string().describe("The workspace ID") }, READ, async ({ workspaceId }) => {
        try {
    ...
    server.tool("dataflow_get", "Get details of a specific Dataflow Gen2 item", {
        workspaceId: z.string().describe("The workspace ID"),
        dataflowId: z.string().describe("The dataflow ID"),
    }, READ, async ({ workspaceId, dataflowId }) => {
        try {
    ...
    server.tool("dataflow_create", "Create a new Dataflow Gen2 item in a workspace", {
        workspaceId: z.string().describe("The workspace ID"),
        ...
        try {
            ...
            const body = { displayName, type: "DataflowGen2" };
            if (description)
    ...
    server.tool("dataflow_update", "Update a Dataflow Gen2 item's name or description", {
        workspaceId: z.string().describe("The workspace ID"),
        dataflowId: z.string().describe("The dataflow ID"),
        displayName: z.string().optional().describe("New display name"),
        description: z.string().optional().describe("New description"),
    }, WRITE, async ({ workspaceId, dataflowId, displayName, description }) => {
        try {
            await workspaceGuard.assertWorkspaceAllowed(fabricClient, workspaceId);
            const body = {};
    ...
    server.tool("dataflow_delete", "Delete a Dataflow Gen2 item", {
        workspaceId: z.string().describe("The workspace ID"),
        dataflowId: z.string().describe("The dataflow ID"),
    }, DESTRUCTIVE, async ({ workspaceId, dataflowId }) => {
        try {
            await workspaceGuard.assertWorkspaceAllowed(fabricClient, workspaceId);
    ...
    server.tool("dataflow_refresh", "Trigger a refresh of a Dataflow Gen2 item", {
        workspaceId: z.string().describe("The workspace ID"),
        dataflowId: z.string().describe("The dataflow ID"),
    }, WRITE, async ({ workspaceId, dataflowId }) => {
        try {
    ...
    server.tool("dataflow_get_refresh_status", "Get the status of a dataflow refresh job", {
        workspaceId: z.string().describe("The workspace ID"),
        dataflowId: z.string().describe("The dataflow ID"),
        jobInstanceId: z.string().describe("The job instance ID from dataflow_refresh"),
    }, READ, async ({ workspaceId, dataflowId, jobInstanceId }) => {
        try {
    ...
    server.tool("dataflow_get_definition", "Get the definition of a Dataflow Gen2 item (long-running). Writes definition files to the specified output directory.", {
        workspaceId: z.string().describe("The workspace ID"),
        dataflowId: z.string().describe("The dataflow ID"),
        outputDirectoryPath: z.string().describe("Directory path where definition files will be written"),
    }, READ, async ({ workspaceId, dataflowId, outputDirectoryPath }) => {
        try {
            const response = await fabricClient.post(`/workspaces/${workspaceId}/items/${dataflowId}/getDefinition`);
            let parts;
            if (response.lro) {
