...
export function registerVariableLibraryTools(server, fabricClient, workspaceGuard) {
    server.tool("variable_library_list", "List all variable libraries in a workspace", { workspaceId: z.string().describe("The workspace ID") }, READ, async ({ workspaceId }) => {
        try {
    ...
    server.tool("variable_library_get", "Get details of a specific variable library, including its active value set name", {
        workspaceId: z.string().describe("The workspace ID"),
        variableLibraryId: z.string().describe("The variable library ID"),
    }, READ, async ({ workspaceId, variableLibraryId }) => {
        try {
    ...
    server.tool("variable_library_create", "Create a new variable library in a workspace (long-running). Optionally provide a directory containing the definition files (variables.json, settings.json, valueSets/*.json).", {
        workspaceId: z.string().describe("The workspace ID"),
    ...
    server.tool("variable_library_update", "Update a variable library's name, description, or active value set", {
        workspaceId: z.string().describe("The workspace ID"),
        variableLibraryId: z.string().describe("The variable library ID"),
    ...
    }, WRITE, async ({ workspaceId, variableLibraryId, displayName, description, activeValueSetName }) => {
        try {
            await workspaceGuard.assertWorkspaceAllowed(fabricClient, workspaceId);
            const body = {};
            ...
            if (description !== undefined)
                body.description = description;
            if (activeValueSetName !== undefined)
                body.properties = { activeValueSetName };
    ...
    server.tool("variable_library_delete", "Delete a variable library", {
        workspaceId: z.string().describe("The workspace ID"),
        variableLibraryId: z.string().describe("The variable library ID"),
    }, DESTRUCTIVE, async ({ workspaceId, variableLibraryId }) => {
        try {
            await workspaceGuard.assertWorkspaceAllowed(fabricClient, workspaceId);
    ...
    server.tool("variable_library_get_definition", "Get the definition of a variable library (long-running). Writes all definition files (variables.json, settings.json, valueSets/*.json, .platform) to the specified output directory and returns the list of files written.", {
        workspaceId: z.string().describe("The workspace ID"),
        variableLibraryId: z.string().describe("The variable library ID"),
        outputDirectoryPath: z.string().describe("Directory path where definition files will be written"),
    }, READ, async ({ workspaceId, variableLibraryId, outputDirectoryPath }) => {
        try {
            const response = await fabricClient.post(`/workspaces/${workspaceId}/VariableLibraries/${variableLibraryId}/getDefinition`);
            let parts;
            if (response.lro) {
    ...
    server.tool("variable_library_update_definition", "Update a variable library's definition (long-running). Reads definition files (variables.json, settings.json, valueSets/*.json, .platform) from the specified directory.", {
        workspaceId: z.string().describe("The workspace ID"),
        variableLibraryId: z.string().describe("The variable library ID"),
    ...
    }, WRITE, async ({ workspaceId, variableLibraryId, definitionDirectoryPath }) => {
        try {
            await workspaceGuard.assertWorkspaceAllowed(fabricClient, workspaceId);
            const files = await readFilesFromDirectory(definitionDirectoryPath, [".json", ".platform"]);
            const parts = files.map((f) => ({
            ...
            const response = await fabricClient.post(`/workspaces/${workspaceId}/VariableLibraries/${variableLibraryId}/updateDefinition`, body);
            if (response.lro) {
                const state = await pollOperation(fabricClient, response.lro.operationId);
...
//# sourceMappingURL=variable-library.js.map